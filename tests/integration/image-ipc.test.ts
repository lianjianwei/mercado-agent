...
describe('image generation IPC', () => {
  it('returns an AiImagesResult and propagates generator errors', async () => {
    ...
    const service = { generate: vi.fn(async () => result), getImages: vi.fn(() => null), publishExisting: vi.fn(async () => result) };
    ...
    const failing = { generate: vi.fn(async () => { throw new Error('boom'); }), getImages: vi.fn(() => null), publishExisting: vi.fn(async () => result) };
